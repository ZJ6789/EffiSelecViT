...
def get_head_zeta(model):
    ...
    head_zetas = [z for k in head_zeta for z in k]
    return head_zetas
...
def get_mlp_zeta(model):
    ...
    mlp_zetas = [z for k in mlp_zeta for z in k]
    return mlp_zetas
...
def get_threshold(checkpoint_path, head_prune_ratio, mlp_prune_ratio):
    ...
    if len(head_zeta) == 0:
        print('There is no head_zeta information in the configuration file')
    else:
        print('Start with the head threshold, and the pruning rate is:', head_prune_ratio)
        head_data = []
        for i in range(len(head_zeta)):
            head_data.append(head_zeta[i].squeeze().reshape(-1).numpy().tolist())

        head_data = [z for k in head_data for z in k]
    ...
    if len(mlp_zeta) == 0:
        print('There is no mlp_zeta information in the configuration file')
    else:
        print('Start to calculate the mlp threshold, and the pruning rate is:', mlp_prune_ratio)
        mlp_data = []
        for i in range(len(mlp_zeta)):
            mlp_data.append(mlp_zeta[i].squeeze().reshape(-1).numpy().tolist())

        mlp_data = [z for k in mlp_data for z in k]
